...
async function testRequiredEmailTypesCannotBeMutated() {
  ...
  const fakeDb = {
    async query(sql) {
      ...
      if (/^SELECT \* FROM user_email_preferences/.test(sql)) {
        return { rows: [{ user_id: 7, email_all_off: true, booking_confirmation: false, password_reset: false }] };
      }
      return { rows: [] };
    },
  };
  ...
  assert.strictEqual(
    rowToPrefs({ email_all_off: true, password_reset: false }).password_reset,
    true,
    'stored false values for required email types must read back as enabled'
  );
}
...
function testBookingUpdateConflictPolicy() {
  ...
  } = require('../lib/booking-update-conflicts');
  ...
  assert.strictEqual(
    shouldCheckBookingUpdateConflicts({ currentStatus: 'cancelled', nextStatus: 'confirmed', scheduleChanged: false }),
    true,
    'reactivating a cancelled booking must run conflict checks even without time/resource changes'
  );
  assert.strictEqual(
