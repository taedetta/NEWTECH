...
function isTrainingStaff(user) {
  return ['owner', 'admin', 'instructor'].includes(user?.role) || (!!user?.is_instructor && user?.role !== 'student' && user?.role !== 'renter');
}

function canViewTrainingStudent(user, studentId) {
  return isTrainingStaff(user) || user?.id === studentId;
}
...
router.get('/student-progress', authenticateToken, async (req, res) => {
  try {
    ...
    if (!canViewTrainingStudent(req.user, studentId)) return res.status(403).json({ error: 'Access denied' });
    const programs = await pool.query('SELECT * FROM training_programs ORDER BY id');
    const stages = await pool.query('SELECT * FROM program_stages ORDER BY program_id, order_index');
    let maneuvers = [];
...
router.post('/student-progress', authenticateToken, async (req, res) => {
  try {
    if (!isTrainingStaff(req.user)) return res.status(403).json({ error: 'Only instructors can update progress' });
    ...
    if (!validStatuses.includes(status)) return res.status(400).json({ error: 'Invalid progress status' });
    const s = status;
...
router.put('/enrollment/:id/stage', authenticateToken, async (req, res) => {
  try {
    if (!isTrainingStaff(req.user)) return res.status(403).json({ error: 'Only instructors can update enrollment stages' });
...
router.put('/maneuver-progress', authenticateToken, async (req, res) => {
  try {
    ...
    if (!isTrainingStaff(req.user)) return res.status(403).json({ error: 'Only instructors can update maneuver progress' });
...
router.post('/debriefs', authenticateToken, async (req, res) => {
  try {
    ...
    if (!isTrainingStaff(req.user)) return res.status(403).json({ error: 'Only instructors can create debriefs' });
...
router.post('/milestones', authenticateToken, async (req, res) => {
  try {
    if (!isTrainingStaff(req.user)) return res.status(403).json({ error: 'Only instructors can sign off stages' });
