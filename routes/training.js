...
const TRAINING_STAFF_ROLES = ['owner', 'admin', 'instructor'];
const trainingAdmin = [authenticateToken, requireRole('owner', 'admin')];

function isTrainingStaff(user) {
  return TRAINING_STAFF_ROLES.includes(user?.role) || (user?.is_instructor && user.role !== 'student');
}
...
router.get('/student-progress', authenticateToken, async (req, res) => {
  try {
    ...
    if (!canViewStudentTraining(req.user, studentId)) {
      return res.status(403).json({ error: 'Access denied' });
    }
    const programs = await pool.query('SELECT * FROM training_programs ORDER BY id');
    const stages = await pool.query('SELECT * FROM program_stages ORDER BY program_id, order_index');
...
router.post('/admin/programs', ...trainingAdmin, async (req, res) => {
  try {
...
router.put('/admin/programs/:id', ...trainingAdmin, async (req, res) => {
  try {
...
router.delete('/admin/programs/:id', ...trainingAdmin, async (req, res) => {
...
router.post('/admin/stages', ...trainingAdmin, async (req, res) => {
  try {
...
router.put('/admin/stages/:id', ...trainingAdmin, async (req, res) => {
  try {
...
router.delete('/admin/stages/:id', ...trainingAdmin, async (req, res) => {
  try {
...
router.post('/admin/maneuvers', ...trainingAdmin, async (req, res) => {
  try {
...
router.put('/admin/maneuvers/:id', ...trainingAdmin, async (req, res) => {
  try {
...
router.delete('/admin/maneuvers/:id', ...trainingAdmin, async (req, res) => {
  try {
...
router.put('/admin/stages/reorder', ...trainingAdmin, async (req, res) => {
  try {
