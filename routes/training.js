...
router.get('/program-enrollments', authenticateToken, async (req, res) => {
  try {
    // Staff can view enrollments; students/renters/maintenance cannot access the roster.
...
router.get('/maneuver-progress/:studentId/:enrollmentId', authenticateToken, async (req, res) => {
  try {
    ...
    // Students can only view their own; training staff can view assigned rosters.
    if (!canViewTrainingStudent(req.user, studentId)) return res.status(403).json({ error: 'Access denied' });
