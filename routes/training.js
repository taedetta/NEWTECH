...
function isTrainingStaff(user) {
  return !!user && (
    ['owner', 'admin', 'instructor'].includes(user.role)
    || (user.is_instructor === true && user.role !== 'student')
  );
}
...
function requireTrainingStaff(req, res, next) {
  if (!isTrainingStaff(req.user)) {
    return res.status(403).json({ error: 'Only instructors and admins can access this training record' });
  }
  next();
}
...
// Support both legacy /api/training/admin/* and current /api/admin/training/* mounts.
