...
function canViewStudentTraining(user, studentId) {
  if (isTrainingStaff(user)) return true;
  return ['student', 'renter'].includes(user.role) && user.id === studentId;
}

function canManageStudentTraining(user) {
...
router.post('/student-progress', authenticateToken, async (req, res) => {
  try {
    ...
    if (!canManageStudentTraining(req.user)) return res.status(403).json({ error: 'Only training staff can update progress' });
...
router.put('/enrollment/:id/stage', authenticateToken, async (req, res) => {
  try {
    if (!canManageStudentTraining(req.user)) return res.status(403).json({ error: 'Only training staff can update enrollment stages' });
    ...
    const currentStageId = current_stage_id == null || current_stage_id === ''
      ? null
      : parseInt(current_stage_id, 10);
    if (currentStageId !== null && !Number.isFinite(currentStageId)) {
...
router.post(['/programs', '/admin/programs'], authenticateToken, requireRole('owner', 'admin'), async (req, res) => {
  try {
...
router.put(['/programs/:id(\\d+)', '/admin/programs/:id(\\d+)'], authenticateToken, requireRole('owner', 'admin'), async (req, res) => {
  try {
...
router.delete(['/programs/:id(\\d+)', '/admin/programs/:id(\\d+)'], authenticateToken, requireRole('owner', 'admin'), async (req, res) => {
...
router.post(['/stages', '/admin/stages'], authenticateToken, requireRole('owner', 'admin'), async (req, res) => {
  try {
...
router.put(['/stages/:id(\\d+)', '/admin/stages/:id(\\d+)'], authenticateToken, requireRole('owner', 'admin'), async (req, res) => {
  try {
...
router.delete(['/stages/:id(\\d+)', '/admin/stages/:id(\\d+)'], authenticateToken, requireRole('owner', 'admin'), async (req, res) => {
  try {
...
router.post(['/maneuvers', '/admin/maneuvers'], authenticateToken, requireRole('owner', 'admin'), async (req, res) => {
  try {
...
router.put(['/maneuvers/:id(\\d+)', '/admin/maneuvers/:id(\\d+)'], authenticateToken, requireRole('owner', 'admin'), async (req, res) => {
  try {
...
router.delete(['/maneuvers/:id(\\d+)', '/admin/maneuvers/:id(\\d+)'], authenticateToken, requireRole('owner', 'admin'), async (req, res) => {
  try {
...
router.put(['/stages/reorder', '/admin/stages/reorder'], authenticateToken, requireRole('owner', 'admin'), async (req, res) => {
  try {
...
router.put('/maneuver-progress', authenticateToken, async (req, res) => {
  try {
    ...
    if (!canManageStudentTraining(req.user)) return res.status(403).json({ error: 'Only instructors can update maneuver progress' });
...
router.post('/debriefs', authenticateToken, async (req, res) => {
  try {
    if (!canManageStudentTraining(req.user)) return res.status(403).json({ error: 'Only instructors can create debriefs' });
...
router.post('/milestones', authenticateToken, async (req, res) => {
  try {
    if (!canManageStudentTraining(req.user)) return res.status(403).json({ error: 'Only instructors can sign off stages' });
