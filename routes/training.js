...
function requireTrainingStaff(req, res, next) {
  if (!isTrainingStaff(req.user)) {
    return res.status(403).json({ error: 'Only instructors, admins, or owners can access this training resource' });
  }
  next();
}
...
router.use('/admin', authenticateToken, requireRole('owner', 'admin'));
