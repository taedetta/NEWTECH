...
function parseNonNegativeNumber(val, fieldName, max = 99999) {
...
router.patch('/:id/rate', authenticateToken, async (req, res) => {
  try {
    ...
    const parsedRate = parseNonNegativeNumber(instructor_rate, 'instructor_rate', 10000);
...
router.put('/:id/hours', authenticateToken, async (req, res) => {
  try {
    ...
    if (total_hobbs_hours !== undefined) {
      const parsed = parseNonNegativeNumber(total_hobbs_hours, 'total_hobbs_hours');
      if (parsed.error) return res.status(400).json({ error: parsed.error });
      updates.push(`total_hobbs_hours = $${idx++}`);
      vals.push(parsed.value);
    }
    if (total_tach_hours !== undefined)  {
      const parsed = parseNonNegativeNumber(total_tach_hours, 'total_tach_hours');
      if (parsed.error) return res.status(400).json({ error: parsed.error });
