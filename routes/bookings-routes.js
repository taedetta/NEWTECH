...
router.put('/:id', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    const b = existing.rows[0];
    const isAdmin = ['owner', 'admin'].includes(req.user.role);
    const isHistoricalBooking = !bookingBlocksSchedule(b.status);
    ...
    if (isHistoricalBooking && !canEditHistoricalBooking(req.user, b)) {
      return res.status(403).json({ error: 'Only admins/owners or the assigned instructor can edit completed or cancelled bookings' });
    }
    const isStaffHistoricalEdit = isHistoricalBooking && canEditHistoricalBooking(req.user, b);
    ...
    const statusForUpdate = status !== undefined ? effectiveStatus : null;
    ...
    const timeCheck = validateBookingTimes({
      ...
      skipPastTimeCheck: isAdmin || isStaffHistoricalEdit,
    });
    if (timeCheck.errors.length) return res.status(400).json({ error: timeCheck.errors[0], errors: timeCheck.errors });
