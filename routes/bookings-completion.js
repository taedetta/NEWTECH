...
router.patch('/:id/end-early', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    if (Number.isNaN(endTime.getTime())) return res.status(400).json({ error: 'actual_end_time is invalid' });
    ...
    if (endTime <= originalStart) {
      await client.query('ROLLBACK'); inTxn = false;
      return res.status(400).json({ error: 'actual_end_time must be after the scheduled start time' });
    }
...
router.patch('/:id/complete', authenticateToken, async (req, res) => {
  const client = await pool.connect();
  let inTxn = false;
  const abortTxn = async (status, body) => {
    if (inTxn) {
      await client.query('ROLLBACK').catch(() => {});
      inTxn = false;
    }
  ...
  try {
    ...
    if (bResult.rows.length === 0) return abortTxn(404, { error: 'Booking not found' });
    const b = bResult.rows[0];
    if (b.status !== 'confirmed') return abortTxn(400, { error: 'Only confirmed bookings can be completed' });
    ...
    if (!isAdmin) {
      if (b.instructor_id) {
        ...
        return abortTxn(403, { error: 'Access denied' });
      }
    }
    ...
    if (rlCheck.blocked) {
      return abortTxn(429, { error: `Too many failed Hobbs submissions. Try again in ${rlCheck.retryAfter} seconds.` });
    }

    // Normal path — validate hobbs before starting transaction
    if (hobbs_start == null || hobbs_end == null) return abortTxn(400, { error: 'hobbs_start and hobbs_end are required' });

    // ── Input sanitization: reject NaN, negative, impossibly large values ──
    const hStartErr = validateHobbsValue(hobbs_start, 'hobbs_start');
    if (hStartErr) { recordHobbsFail(req.user.id); return abortTxn(400, { error: hStartErr }); }
    const hEndErr = validateHobbsValue(hobbs_end, 'hobbs_end');
    if (hEndErr) { recordHobbsFail(req.user.id); return abortTxn(400, { error: hEndErr }); }

    const hStart = parseFloat(hobbs_start);
    const hEnd = parseFloat(hobbs_end);
    if (hEnd <= hStart) { recordHobbsFail(req.user.id); return abortTxn(400, { error: 'hobbs_end must be greater than hobbs_start' }); }

    // Validate tach values if provided — both or neither
    if ((tach_start != null) !== (tach_end != null)) {
      return abortTxn(400, { error: 'Both tach_start and tach_end are required when logging tach time' });
    }
    if (tach_start != null) {
      const tStartErr = validateHobbsValue(tach_start, 'tach_start');
      if (tStartErr) return abortTxn(400, { error: tStartErr });
      const tEndErr = validateHobbsValue(tach_end, 'tach_end');
      if (tEndErr) return abortTxn(400, { error: tEndErr });
    }
    if (tach_start != null && tach_end != null && parseFloat(tach_end) <= parseFloat(tach_start)) {
      return abortTxn(400, { error: 'tach_end must be greater than tach_start' });
    }
    ...
    const maxMeterDelta = Math.min(12.5, Math.max(0.5, scheduledHours + 0.5));
    if (hobbsFlown > maxMeterDelta) {
      recordHobbsFail(req.user.id);
      return abortTxn(400, {
        error: `Hobbs delta (${hobbsFlown.toFixed(1)} hrs) exceeds the scheduled flight duration. Verify the meter readings.`,
      });
    }
    ...
    if (dual_instruction_hours != null) {
      ...
      if (dualErr) return abortTxn(400, { error: dualErr });
    }
    const tachFlown = (tStart != null && tEnd != null) ? (tEnd - tStart) : null;
    if (tachFlown != null && tachFlown > maxMeterDelta) {
      return abortTxn(400, {
        error: `Tach delta (${tachFlown.toFixed(1)} hrs) exceeds the scheduled flight duration. Verify the meter readings.`,
      });
    }
