...
const MAX_FLIGHT_METER_DELTA_HOURS = 12;
...
router.patch('/:id/end-early', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    if (endTime <= originalStart) return res.status(400).json({ error: 'actual_end_time must be after the booking start time' });
...
router.patch('/:id/complete', authenticateToken, async (req, res) => {
  const client = await pool.connect();
  let inTx = false;
  const rollbackAndReturn = async (status, body) => {
    if (inTx) {
      await client.query('ROLLBACK');
      inTx = false;
    }
  ...
  try {
    ...
    if (hEnd - hStart > MAX_FLIGHT_METER_DELTA_HOURS) {
      recordHobbsFail(req.user.id);
      return rollbackAndReturn(400, { error: `Hobbs delta cannot exceed ${MAX_FLIGHT_METER_DELTA_HOURS} hours` });
    }
    ...
    if (tStart != null && tEnd != null && tEnd - tStart > MAX_FLIGHT_METER_DELTA_HOURS) {
      return rollbackAndReturn(400, { error: `Tach delta cannot exceed ${MAX_FLIGHT_METER_DELTA_HOURS} hours` });
    }
