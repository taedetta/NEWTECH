...
const MAX_COMPLETION_METER_DELTA = 24; // one completion should not jump a meter by multiple days of flying
...
router.patch('/:id/complete', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    await client.query('BEGIN');
    const reject = async (statusCode, payload) => {
      await client.query('ROLLBACK').catch(() => {});
      return res.status(statusCode).json(payload);
    };

    const bResult = await client.query('SELECT * FROM bookings WHERE id = $1 FOR UPDATE', [req.params.id]);
    if (bResult.rows.length === 0) return reject(404, { error: 'Booking not found' });
    const b = bResult.rows[0];
    if (b.status !== 'confirmed') return reject(400, { error: 'Only confirmed bookings can be completed' });
    ...
    if (!isAdmin) {
      if (b.instructor_id) {
        ...
        return reject(403, { error: 'Access denied' });
      }
    }
    ...
    if (rlCheck.blocked) {
      return reject(429, { error: `Too many failed Hobbs submissions. Try again in ${rlCheck.retryAfter} seconds.` });
    }

    // Normal path — validate hobbs before starting transaction
    if (hobbs_start == null || hobbs_end == null) return reject(400, { error: 'hobbs_start and hobbs_end are required' });

    // ── Input sanitization: reject NaN, negative, impossibly large values ──
    const hStartErr = validateHobbsValue(hobbs_start, 'hobbs_start');
    if (hStartErr) { recordHobbsFail(req.user.id); return reject(400, { error: hStartErr }); }
    const hEndErr = validateHobbsValue(hobbs_end, 'hobbs_end');
    if (hEndErr) { recordHobbsFail(req.user.id); return reject(400, { error: hEndErr }); }

    const hStart = parseFloat(hobbs_start);
    const hEnd = parseFloat(hobbs_end);
    if (hEnd <= hStart) { recordHobbsFail(req.user.id); return reject(400, { error: 'hobbs_end must be greater than hobbs_start' }); }
    if (hEnd - hStart > MAX_COMPLETION_METER_DELTA) {
      recordHobbsFail(req.user.id);
      return reject(400, { error: `Hobbs time cannot exceed ${MAX_COMPLETION_METER_DELTA} hours for one completion` });
    }

    // Validate tach values if provided — both or neither
    if ((tach_start != null) !== (tach_end != null)) {
      return reject(400, { error: 'Both tach_start and tach_end are required when logging tach time' });
    }
    if (tach_start != null) {
      const tStartErr = validateHobbsValue(tach_start, 'tach_start');
      if (tStartErr) return reject(400, { error: tStartErr });
      const tEndErr = validateHobbsValue(tach_end, 'tach_end');
      if (tEndErr) return reject(400, { error: tEndErr });
    }
    if (tach_start != null && tach_end != null && parseFloat(tach_end) <= parseFloat(tach_start)) {
      return reject(400, { error: 'tach_end must be greater than tach_start' });
    }
    if (tach_start != null && parseFloat(tach_end) - parseFloat(tach_start) > MAX_COMPLETION_METER_DELTA) {
      return reject(400, { error: `Tach time cannot exceed ${MAX_COMPLETION_METER_DELTA} hours for one completion` });
    }
    ...
    if (dual_instruction_hours != null) {
      ...
      if (dualErr) return reject(400, { error: dualErr });
    }
