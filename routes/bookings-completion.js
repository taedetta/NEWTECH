...
router.patch('/:id/end-early', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    if (result.rows.length === 0) return rollbackJson(client, res, 404, { error: 'Booking not found' });
    const b = result.rows[0];
    const isAdmin = ['owner', 'admin'].includes(req.user.role);
    if (!isAdmin && req.user.id !== b.instructor_id && req.user.id !== b.student_id) {
      return rollbackJson(client, res, 403, { error: 'Access denied' });
    }
    if (b.status !== 'confirmed') return rollbackJson(client, res, 400, { error: 'Only confirmed bookings can be ended early' });
    const startTime = new Date(b.start_time);
    if (endTime <= startTime) {
      return rollbackJson(client, res, 400, { error: 'actual_end_time must be after the booking start time' });
    }
    const originalEnd = new Date(b.end_time);
    if (endTime >= originalEnd) return rollbackJson(client, res, 400, { error: 'actual_end_time must be before the scheduled end time' });
...
router.patch('/:id/complete', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    if (bResult.rows.length === 0) return rollbackJson(client, res, 404, { error: 'Booking not found' });
    const b = bResult.rows[0];
    if (b.status !== 'confirmed') return rollbackJson(client, res, 400, { error: 'Only confirmed bookings can be completed' });
    const isAdmin = ['owner', 'admin'].includes(verifiedRole);
    if (!isAdmin) {
      if (b.instructor_id) {
        if (req.user.id !== b.instructor_id) {
          return rollbackJson(client, res, 403, {
    ...
    if (rlCheck.blocked) {
      return rollbackJson(client, res, 429, { error: `Too many failed Hobbs submissions. Try again in ${rlCheck.retryAfter} seconds.` });
    }

    // Normal path — validate hobbs before starting transaction
    if (hobbs_start == null || hobbs_end == null) return rollbackJson(client, res, 400, { error: 'hobbs_start and hobbs_end are required' });

    // ── Input sanitization: reject NaN, negative, impossibly large values ──
    const hStartErr = validateHobbsValue(hobbs_start, 'hobbs_start');
    if (hStartErr) { recordHobbsFail(req.user.id); return rollbackJson(client, res, 400, { error: hStartErr }); }
    const hEndErr = validateHobbsValue(hobbs_end, 'hobbs_end');
    if (hEndErr) { recordHobbsFail(req.user.id); return rollbackJson(client, res, 400, { error: hEndErr }); }

    const hStart = parseFloat(hobbs_start);
    const hEnd = parseFloat(hobbs_end);
    if (hEnd <= hStart) { recordHobbsFail(req.user.id); return rollbackJson(client, res, 400, { error: 'hobbs_end must be greater than hobbs_start' }); }

    // Validate tach values if provided — both or neither
    if ((tach_start != null) !== (tach_end != null)) {
      return rollbackJson(client, res, 400, { error: 'Both tach_start and tach_end are required when logging tach time' });
    }
    if (tach_start != null) {
      const tStartErr = validateHobbsValue(tach_start, 'tach_start');
      if (tStartErr) return rollbackJson(client, res, 400, { error: tStartErr });
      const tEndErr = validateHobbsValue(tach_end, 'tach_end');
      if (tEndErr) return rollbackJson(client, res, 400, { error: tEndErr });
    }
    if (tach_start != null && tach_end != null && parseFloat(tach_end) <= parseFloat(tach_start)) {
      return rollbackJson(client, res, 400, { error: 'tach_end must be greater than tach_start' });
    }
      ...
      if (acResult.rows.length > 0) {
        ...
        if (currentHobbs != null && currentHobbs > 0) {
          if (hStart < currentHobbs - 0.1) {
            ...
            return rollbackJson(client, res, 400, {
    ...
    if (dual_instruction_hours != null) {
      ...
      if (dualErr) return rollbackJson(client, res, 400, { error: dualErr });
    }
...
router.get('/:id', authenticateToken, async (req, res) => {
  try {
    ...
    if (!isStaff && req.user.id !== booking.student_id && req.user.id !== booking.instructor_id) {
