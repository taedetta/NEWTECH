...
function parseOptionalNonNegative(val, fieldName, max = 99999) {
...
router.post('/', authenticateToken, requirePermission('can_manage_aircraft'), async (req, res) => {
  try {
    ...
    const rate = parseOptionalNonNegative(hourly_rate, 'hourly_rate', 10000);
...
router.put('/:id', authenticateToken, requirePermission('can_manage_aircraft'), async (req, res) => {
  try {
    ...
    const result = await pool.query(
      ...
      [tail_number?.toUpperCase(), make_model, type, year, hourly_rate !== undefined ? rate.value : null, status, notes, req.params.id]
    );
...
router.patch('/:id/hobbs', authenticateToken, async (req, res) => {
  ...
  const hobbsParsed = hobbs != null ? parseOptionalNonNegative(hobbs, 'hobbs') : { value: null };
  if (hobbsParsed.error) return res.status(400).json({ error: hobbsParsed.error });
  const tachParsed = tach != null ? parseOptionalNonNegative(tach, 'tach') : { value: null };
  if (tachParsed.error) return res.status(400).json({ error: tachParsed.error });
  const client = await pool.connect();
  try {
    ...
    let idx = 1;
    if (hobbs != null) {
      const hVal = hobbsParsed.value;
      sets.push(`total_hobbs_hours = $${idx++}`, `current_hobbs = $${idx++}`);
      vals.push(hVal, hVal);
    }
    if (tach != null) {
      const tVal = tachParsed.value;
    ...
    if (hobbs != null) {
      await client.query(
        ...
        [req.params.id, req.user.id, getMeterHobbs(acRow), Number(hobbs), note || null]
      );
    }
    if (tach != null) {
      await client.query(
        ...
        [req.params.id, req.user.id, getMeterTach(acRow), Number(tach), note || null]
      );
    }
    await client.query('COMMIT');
    res.json(result.rows[0]);
...
router.put('/:id/inspections', authenticateToken, requirePermission('can_manage_aircraft'), async (req, res) => {
  try {
    ...
    const due100 = parseOptionalNonNegative(next_100hr_due, 'next_100hr_due');
    if (due100.error) return res.status(400).json({ error: due100.error });
    const result = await pool.query(
      `UPDATE aircraft
...
router.post('/:id/ads', authenticateToken, requirePermission('can_manage_aircraft'), async (req, res) => {
  try {
    ...
    const dueHobbsVal = parseOptionalNonNegative(due_hobbs, 'due_hobbs');
    if (dueHobbsVal.error) return res.status(400).json({ error: dueHobbsVal.error });
    const result = await pool.query(
      `INSERT INTO airworthiness_directives (aircraft_id, ad_number, description, due_date, due_hobbs)
       VALUES ($1, $2, $3, $4, $5) RETURNING *`,
      [req.params.id, ad_number || null, description, due_date || null, dueHobbsVal.value]
    );
    res.status(201).json(result.rows[0]);
  } catch (err) {
...
router.patch('/:id/ads/:adId', authenticateToken, requirePermission('can_manage_aircraft'), async (req, res) => {
  try {
    ...
    const result = await pool.query(
      ...
      [status || null, description || null, due_date || null, due_hobbs !== undefined ? dueHobbsVal.value : null, ad_number || null, req.params.adId, req.params.id]
    );
