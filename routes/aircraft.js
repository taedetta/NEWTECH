...
router.post('/', authenticateToken, requirePermission('can_manage_aircraft'), async (req, res) => {
  try {
    ...
    const parsedHourlyRate = parseOptionalNonNegativeNumber(hourly_rate, 'hourly_rate', 9999);
    if (parsedHourlyRate.error) return res.status(400).json({ error: parsedHourlyRate.error });
    const result = await pool.query(
      `INSERT INTO aircraft (tail_number, make_model, type, year, hourly_rate, notes)
       VALUES ($1, $2, $3, $4, $5, $6) RETURNING *`,
      [tail_number.toUpperCase(), make_model, type || 'single_engine', year, parsedHourlyRate.value, notes]
    );
...
router.put('/:id', authenticateToken, requirePermission('can_manage_aircraft'), async (req, res) => {
  try {
    ...
    const result = await pool.query(
      ...
      [tail_number?.toUpperCase(), make_model, type, year, parsedHourlyRate.value, status, notes, req.params.id]
    );
...
router.patch('/:id/hobbs', authenticateToken, async (req, res) => {
  ...
  const parsedHobbs = parseOptionalNonNegativeNumber(hobbs, 'hobbs');
  if (parsedHobbs.error) return res.status(400).json({ error: parsedHobbs.error });
  const parsedTach = parseOptionalNonNegativeNumber(tach, 'tach');
...
router.put('/:id/inspections', authenticateToken, requirePermission('can_manage_aircraft'), async (req, res) => {
  try {
    ...
    const parsed100HrDue = parseOptionalNonNegativeNumber(next_100hr_due, 'next_100hr_due');
    if (parsed100HrDue.error) return res.status(400).json({ error: parsed100HrDue.error });
    const result = await pool.query(
      `UPDATE aircraft
       SET next_100hr_due = $1, next_annual_due = $2, updated_at = NOW()
       WHERE id = $3 RETURNING *`,
      [
        parsed100HrDue.value,
...
router.post('/:id/ads', authenticateToken, requirePermission('can_manage_aircraft'), async (req, res) => {
  try {
    ...
    const parsedDueHobbs = parseOptionalNonNegativeNumber(due_hobbs, 'due_hobbs');
    if (parsedDueHobbs.error) return res.status(400).json({ error: parsedDueHobbs.error });
    const result = await pool.query(
      `INSERT INTO airworthiness_directives (aircraft_id, ad_number, description, due_date, due_hobbs)
       VALUES ($1, $2, $3, $4, $5) RETURNING *`,
      [req.params.id, ad_number || null, description, due_date || null, parsedDueHobbs.value]
    );
    res.status(201).json(result.rows[0]);
  } catch (err) {
...
router.patch('/:id/ads/:adId', authenticateToken, requirePermission('can_manage_aircraft'), async (req, res) => {
  try {
    ...
    const result = await pool.query(
      ...
      [status || null, description || null, due_date || null, parsedDueHobbs.value, ad_number || null, req.params.adId, req.params.id]
    );
