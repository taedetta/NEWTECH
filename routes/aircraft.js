...
router.patch('/:id/hobbs', authenticateToken, requirePermission('can_manage_aircraft'), async (req, res) => {
  ...
  if ((hobbs == null || hobbs === '') && (tach == null || tach === '')) {
...
router.put('/:id/inspections', authenticateToken, requirePermission('can_manage_aircraft'), async (req, res) => {
  try {
    ...
    const next100hrDue = parseStrictNonNegativeNumber(next_100hr_due, 'next_100hr_due');
    const result = await pool.query(
      `UPDATE aircraft
    ...
    res.status(err.status || 500).json({ error: err.status ? err.message : 'Failed to update inspections' });
  }
});
...
router.post('/:id/ads', authenticateToken, requirePermission('can_manage_aircraft'), async (req, res) => {
  try {
    ...
    const dueHobbs = parseStrictNonNegativeNumber(due_hobbs, 'due_hobbs');
    const result = await pool.query(
    ...
    res.status(err.status || 500).json({ error: err.status ? err.message : 'Failed to create AD' });
  }
});
...
router.patch('/:id/ads/:adId', authenticateToken, requirePermission('can_manage_aircraft'), async (req, res) => {
  try {
    ...
    const dueHobbs = due_hobbs !== undefined ? parseStrictNonNegativeNumber(due_hobbs, 'due_hobbs') : null;
    const result = await pool.query(
    ...
    res.status(err.status || 500).json({ error: err.status ? err.message : 'Failed to update AD' });
  }
});
