...
router.post('/', authenticateToken, requirePermission('can_manage_aircraft'), async (req, res) => {
  try {
    ...
    const rate = parseStrictNumber(hourly_rate, 'hourly_rate');
...
router.patch('/:id/hobbs', authenticateToken, async (req, res) => {
  ...
  const hobbsParsed = parseStrictNumber(hobbs, 'hobbs');
  if (hobbsParsed.error) return res.status(400).json({ error: hobbsParsed.error });
  const tachParsed = parseStrictNumber(tach, 'tach');
  ...
  try {
    ...
    if (hobbs != null) {
      await client.query(
        ...
        [req.params.id, req.user.id, getMeterHobbs(acRow), hobbsParsed.value, note || null]
      );
    }
    if (tach != null) {
      await client.query(
        ...
        [req.params.id, req.user.id, getMeterTach(acRow), tachParsed.value, note || null]
      );
    }
    await client.query('COMMIT');
    res.json(result.rows[0]);
...
router.put('/:id/inspections', authenticateToken, requirePermission('can_manage_aircraft'), async (req, res) => {
  try {
    ...
    const due100 = parseStrictNumber(next_100hr_due, 'next_100hr_due');
...
router.post('/:id/ads', authenticateToken, requirePermission('can_manage_aircraft'), async (req, res) => {
  try {
    ...
    const dueHobbsVal = parseStrictNumber(due_hobbs, 'due_hobbs');
...
router.patch('/:id/ads/:adId', authenticateToken, requirePermission('can_manage_aircraft'), async (req, res) => {
  try {
    ...
    const result = await pool.query(
      ...
      [status || null, description || null, due_date || null, dueHobbsVal.value, ad_number || null, req.params.adId, req.params.id]
    );
