...
router.post('/login', async (req, res) => {
  try {
    ...
    if (user.deleted_at || user.approval_status === 'rejected') {
      return res.status(403).json({ error: 'account_inactive', message: 'This account is no longer active.' });
    }
...
router.post('/claim-owner', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    await client.query('SELECT pg_advisory_xact_lock($1, $2)', [99, 1]);
    const ownerCheck = await client.query("SELECT id FROM users WHERE role = 'owner' AND deleted_at IS NULL");
    if (ownerCheck.rows.length > 0) {
      await client.query('ROLLBACK');
      return res.status(409).json({ error: 'An owner already exists' });
    }
    const currentRole = await client.query(
      `SELECT role, approval_status, deleted_at
       FROM users
       WHERE id = $1`,
      [req.user.id]
    );
    const currentUser = currentRole.rows[0];
    if (!currentUser || currentUser.deleted_at || currentUser.approval_status !== 'approved') {
      await client.query('ROLLBACK');
      return res.status(401).json({ error: 'Account is not active' });
    }
    if (['student', 'renter'].includes(currentUser.role)) {
      await client.query('ROLLBACK');
