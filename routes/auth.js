...
router.post('/login', async (req, res) => {
  try {
    ...
    const user = result.rows[0];
    if (user.deleted_at || !user.password_hash) {
      return res.status(401).json({ error: 'Invalid email or password' });
    }
    ...
    if (user.approval_status === 'rejected') {
      return res.status(403).json({ error: 'account_rejected', message: 'This account has been rejected.' });
    }
...
router.post('/claim-owner', authenticateToken, async (req, res) => {
  try {
    ...
    const currentRole = await pool.query('SELECT role, email FROM users WHERE id = $1', [req.user.id]);
    if (!isPlatformAdminEmail(currentRole.rows[0]?.email)) {
      return res.status(403).json({ error: 'Only the platform administrator can claim the initial owner role' });
    }
    const result = await pool.query(
