...
router.post('/login', async (req, res) => {
  try {
    ...
    if (user.approval_status === 'rejected') {
      return res.status(403).json({ error: 'account_rejected', message: 'Your account request was not approved.' });
    }
...
router.get('/me', authenticateToken, async (req, res) => {
  try {
    ...
    if (u.approval_status && u.approval_status !== 'approved') {
      return res.status(403).json({ error: u.approval_status === 'pending' ? 'pending_approval' : 'account_not_approved' });
    }
...
router.post('/claim-owner', authenticateToken, async (req, res) => {
  try {
    ...
    if (!current || current.deleted_at || current.approval_status !== 'approved') {
      return res.status(403).json({ error: 'Account is not eligible to claim owner role' });
    }
    if (!isPlatformAdminEmail(current.email)) {
