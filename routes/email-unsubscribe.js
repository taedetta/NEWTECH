...
const { verifyUnsubscribeToken, typeLabel, buildManagePrefsUrl, normalizeUnsubscribeType } = require('../lib/unsubscribe-token');
...
router.get('/unsubscribe', async (req, res) => {
  try {
    ...
    const safeType = normalizeUnsubscribeType(rawType);
    if (!safeType) {
      return res.status(400).send(renderPage({
        ok: false,
    ...
    const verified = verifyUnsubscribeToken(token, safeType);
    if (!verified) {
      return res.status(400).send(renderPage({
        ok: false,
    ...
    if (safeType === 'all') {
      ...
      await updatePrefs(verified.userId, { [safeType]: false });
    }

    const label = typeLabel(safeType);
    return res.send(renderPage({
      ok: true,
      title: 'Unsubscribed',
      message: safeType === 'all'
