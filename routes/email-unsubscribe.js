...
function readUnsubscribeRequest(req) {
  ...
  const queryType = String(req.body?.type || req.query.type || '').trim();
  if (!token) return { error: 'missing' };

  const verified = verifyUnsubscribeToken(token);
  if (!verified) return { error: 'invalid' };

  const rawType = verified.type || (verified.legacyTypeInQuery ? (queryType || 'all') : '');
  if (!rawType || (verified.type && queryType && queryType !== verified.type)) {
