  ...
  try {
    ...
    if (verified.type && verified.type !== rawType) {
      return res.status(400).send(renderPage({
        ok: false,
        title: 'Invalid preference type',
        message: 'This unsubscribe link is only valid for its original email type. Sign in and open My Account to manage your email preferences.',
      }));
    }

    if (isRequiredEmailType(rawType)) {
      return res.status(400).send(renderPage({
        ok: false,
        title: 'Required email',
        message: 'Password reset emails are required for account recovery and cannot be disabled.',
      }));
    }
