...
async function applyAircraftMeterReadings(client, aircraftId, { hobbsEnd, tachEnd, bookingId = null, source = 'flight_complete' }) {
  ...
  if (nextTach != null) {
    await client.query(
      `UPDATE aircraft SET total_hobbs_hours = $1, current_hobbs = $1, total_tach_hours = $2, current_tach = $2, updated_at = NOW() WHERE id = $3 AND source = $4`,
      [nextHobbs, nextTach, aircraftId, getAppEnv()]
    );
    if (nextTach !== oldTach) {
      await client.query(
    ...
    await client.query(
      `UPDATE aircraft SET total_hobbs_hours = $1, current_hobbs = $1, updated_at = NOW() WHERE id = $2 AND source = $3`,
      [nextHobbs, aircraftId, getAppEnv()]
    );
  }
...
async function maxRemainingMeterValue(client, aircraftId, bookingId, field) {
  const history = await client.query(
    ...
    [aircraftId, field, bookingId, getAppEnv()]
  );
  ...
  const logs = await client.query(
    ...
    [aircraftId, bookingId, getAppEnv()]
  );
  return maxFinite([
...
async function rollbackAircraftMeterForDeletedBooking(client, aircraftId, bookingId, log = null) {
  ...
  params.push(aircraftId, getAppEnv());
  await client.query(`UPDATE aircraft SET ${sets.join(', ')} WHERE id = $${idx} AND source = $${idx + 1}`, params);
}

async function reconcileMeterFieldForFlightEdit(client, aircraftId, bookingId, field, aircraft, { oldEnd, newEnd, source }) {
  const previousEnd = toFiniteNumber(oldEnd);
  const submittedEnd = toFiniteNumber(newEnd);
  if (previousEnd == null || submittedEnd == null || submittedEnd >= previousEnd) return null;

  const current = field === 'tach' ? getMeterTach(aircraft) : getMeterHobbs(aircraft);
  if (current == null || !nearlyEqual(current, previousEnd)) return null;

  const remainingMax = await maxRemainingMeterValue(client, aircraftId, bookingId, field);
  const replacement = maxFinite([submittedEnd, remainingMax, 0]);
  if (replacement == null || replacement >= current) return null;

  await client.query(
    `INSERT INTO aircraft_hours_history (aircraft_id, booking_id, field, old_value, new_value, source)
     VALUES ($1, $2, $3, $4, $5, $6)`,
    [aircraftId, bookingId, field, current, replacement, source || 'flight_record_sync_correction']
  );
  return replacement;
}

async function reconcileAircraftMeterForFlightEdit(client, aircraftId, bookingId, {
  oldHobbsEnd,
  oldTachEnd,
  newHobbsEnd,
  newTachEnd,
  ...
  if (acResult.rows.length === 0) return;
  const aircraft = acResult.rows[0];
  const nextHobbs = await reconcileMeterFieldForFlightEdit(client, aircraftId, bookingId, 'hobbs', aircraft, {
    oldEnd: oldHobbsEnd,
    newEnd: newHobbsEnd,
    source,
  });
  const nextTach = await reconcileMeterFieldForFlightEdit(client, aircraftId, bookingId, 'tach', aircraft, {
    oldEnd: oldTachEnd,
    newEnd: newTachEnd,
    source,
  });
