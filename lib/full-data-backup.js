...
const { formatDateTimeShort } = require('./school-timezone');
...
function buildZipBufferFromFiles(files, readme) {
...
function buildBackupEmail({ dateLabel, reason, triggeredBy, files, zipName }) {
  const generatedAt = formatDateTimeShort(new Date());
...
<p style="margin:0 0 4px;color:#6B7280;font-size:13px;">Generated: <strong>${generatedAt}</strong></p>
...
async function emailFullDataBackup(pool, { reason = 'manual', triggeredBy = null, recipients = [] } = {}) {
  ...
  const zipBuffer = await buildZipBufferFromFiles(files, readme);
