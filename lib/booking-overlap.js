...
function isHistoricalScheduleEdit(user, booking) {
  const isHistoricalBooking = booking?.status === 'completed' || booking?.status === 'cancelled';
  if (!isHistoricalBooking) return false;
  const isAdmin = user?.role === 'owner' || user?.role === 'admin';
  const isAssignedInstructor = user?.role === 'instructor' && user?.id === booking?.instructor_id;
  return isAdmin || isAssignedInstructor;
}
