...
const { EMAIL_TYPES, REQUIRED_EMAIL_TYPES, TYPE_LABELS, TYPE_CATEGORIES } = require('./email-types');
...
function appendUnsubscribeFooter(html, text, userId, emailType) {
  ...
  if (REQUIRED_EMAIL_TYPES.has(emailType)) return { html, text };
  ...
  let newHtml = html;
  if (typeof html === 'string' && html.includes('<!-- Footer -->')) {
    newHtml = html.replace('          <!-- Footer -->', `${footerHtml}\n          <!-- Footer -->`);
  } else if (typeof html === 'string' && html.includes('</body>')) {
...
async function shouldSendEmail(userId, type) {
  ...
  if (REQUIRED_EMAIL_TYPES.has(type)) return true;
  try {
