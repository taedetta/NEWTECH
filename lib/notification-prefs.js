...
function getPreferenceCatalog(role, isInstructor) {
  ...
  for (const type of Object.keys(EMAIL_TYPES)) {
    if (isRequiredEmailType(type)) visible.delete(type);
  }
  ...
  const footerHtml = `
              <div style="margin-top:28px;padding-top:20px;border-top:1px solid #e2e8f0;text-align:center;">
                ...
                <p style="margin:0;font-size:12px;color:#64748b;line-height:1.6;">
                  ...
                  &nbsp;·&nbsp;
                  <a href="${unsubAllUrl}" style="color:#64748b;text-decoration:underline;">Unsubscribe from optional emails</a>
                  &nbsp;·&nbsp;
...
function isEmailEnabledByPrefs(type, prefs) {
  if (!type || !EMAIL_TYPES[type]) return true;
...
async function shouldSendEmail(userId, type) {
  ...
  try {
    ...
    return isEmailEnabledByPrefs(type, prefs);
  } catch (err) {
