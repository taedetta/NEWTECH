...
const OPTIONAL_TYPE_SET = new Set(Object.values(OPTIONAL_EMAIL_TYPES));
const REQUIRED_TYPE_SET = new Set(Object.values(REQUIRED_EMAIL_TYPES));
...
function appendUnsubscribeFooter(html, text, userId, emailType) {
  if (!userId || !emailType || !OPTIONAL_TYPE_SET.has(emailType)) return { html, text };
...
async function shouldSendEmail(userId, type) {
  ...
  if (REQUIRED_TYPE_SET.has(type)) return true;
  try {
