...
function getPreferenceCatalog(role, isInstructor) {
  ...
  for (const key of Object.keys(EMAIL_TYPES)) {
    if (!isConfigurableEmailType(key)) visible.delete(key);
  }
...
function isEmailSuppressedByPrefs(prefs, type) {
  if (isRequiredEmailType(type)) return false;
  if (prefs.email_all_off) return true;
...
async function shouldSendEmail(userId, type) {
  ...
  try {
    ...
    return !isEmailSuppressedByPrefs(prefs, type);
  } catch (err) {
