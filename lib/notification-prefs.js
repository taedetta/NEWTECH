...
const { EMAIL_TYPES, TYPE_LABELS, TYPE_CATEGORIES, isMandatoryEmailType } = require('./email-types');
...
function getPreferenceCatalog(role, isInstructor) {
  ...
  for (const key of Object.keys(EMAIL_TYPES)) {
    if (isMandatoryEmailType(key)) visible.delete(key);
  }
...
function appendUnsubscribeFooter(html, text, userId, emailType) {
  ...
  if (isMandatoryEmailType(emailType)) return { html, text };
...
async function shouldSendEmail(userId, type) {
  ...
  if (isMandatoryEmailType(type)) return true;
  try {
