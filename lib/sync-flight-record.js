...
const { shiftedBookingTimesForDate, toDateOnly } = require('./booking-date-shift');
...
async function syncFlightRecord(client, bookingId, patch = {}) {
  ...
  const flightDate = toDateOnly(patch.flight_date)
    || toDateOnly(oldLog?.flight_date)
    || toDateOnly(bookingBefore.start_time)
  ...
  const { aircraftChargeAmount, instructionChargeAmount } = resolveFlightCharges({
    lessonType,
    bookingType: bookingBefore.booking_type,
  ...
  if (patch.flight_date) {
    const shiftedTimes = shiftedBookingTimesForDate(bookingBefore.start_time, bookingBefore.end_time, flightDate);
    if (shiftedTimes) {
      bookingSets.push(`start_time = $${bi++}`);
      bookingVals.push(shiftedTimes.startTime);
      bookingSets.push(`end_time = $${bi++}`);
      bookingVals.push(shiftedTimes.endTime);
    }
  }
