...
async function syncFlightRecord(client, bookingId, patch = {}) {
  ...
  const flightDate = patch.flight_date
    || oldLog?.flight_date
    || (bookingBefore.start_time ? calendarDateFromDate(bookingBefore.start_time) : null)
  ...
  if (patch.flight_date) {
    const currentStart = new Date(bookingBefore.start_time);
    const currentEnd = new Date(bookingBefore.end_time);
    const durationMs = Number.isFinite(currentStart.getTime()) && Number.isFinite(currentEnd.getTime())
      ? currentEnd.getTime() - currentStart.getTime()
      : 60 * 60 * 1000;
    const startHm = Number.isFinite(currentStart.getTime()) ? timeHmFromDate(currentStart) : '12:00';
    const startTime = wallClockToUtc(flightDate, startHm);
    const endTime = new Date(startTime.getTime() + (durationMs > 0 ? durationMs : 60 * 60 * 1000));
...
async function syncFlightRecordFromInstructorHours(client, instructorHoursRow, options = {}) {
  if (!instructorHoursRow.booking_id) return null;
  const preserveRateOverrides = options.preserveRateOverrides === true;

  const bkRes = await client.query(
    'SELECT hobbs_start, hobbs_end, start_time FROM bookings WHERE id = $1',
    [instructorHoursRow.booking_id]
  );
  const bookingRow = bkRes.rows[0] || {};
  const entryDate = instructorHoursRow.entry_date instanceof Date
    ? calendarDateFromDate(instructorHoursRow.entry_date)
    : (instructorHoursRow.entry_date ? String(instructorHoursRow.entry_date).slice(0, 10) : null);
  const bookingDate = bookingRow.start_time ? calendarDateFromDate(bookingRow.start_time) : null;
  const patch = {
    dual_instruction_hours: instructorHoursRow.instruction_hours,
    preserve_instructor_hours_rates: preserveRateOverrides,
  };
  if (entryDate && entryDate !== bookingDate) patch.flight_date = entryDate;
  if (preserveRateOverrides) {
