...
async function syncFlightRecord(client, bookingId, patch = {}) {
  ...
  if ((hStart != null && hStart < 0) || (hEnd != null && hEnd < 0)) {
    throw new Error('Hobbs values cannot be negative');
  }
  if (hStart != null && hEnd != null && hEnd <= hStart) {
    throw new Error('hobbs_end must be greater than hobbs_start');
  }
  if ((tStart != null && tStart < 0) || (tEnd != null && tEnd < 0)) {
    throw new Error('Tach values cannot be negative');
  }
  if (tStart != null && tEnd != null && tEnd <= tStart) {
    throw new Error('tach_end must be greater than tach_start');
  }
  ...
  if (dualHrs < 0) throw new Error('dual_instruction_hours cannot be negative');
