...
function parseInputNum(v, fieldName) {
  if (v == null || v === '') return null;
...
async function syncFlightRecord(client, bookingId, patch = {}) {
  ...
  if (hStart != null && hEnd != null && hEnd <= hStart) {
    const err = new Error('hobbs_end must be greater than hobbs_start');
    err.status = 400;
    throw err;
  }
  if (tStart != null && tEnd != null && tEnd <= tStart) {
    const err = new Error('tach_end must be greater than tach_start');
    err.status = 400;
    throw err;
  }
  if (hobbsDelta != null && hobbsDelta > 12) {
    const err = new Error('hobbs delta exceeds maximum allowed value');
    err.status = 400;
    throw err;
  }
  if (tachDelta != null && tachDelta > 12) {
    const err = new Error('tach delta exceeds maximum allowed value');
    err.status = 400;
    throw err;
  }

  const dualHrs = patch.dual_instruction_hours !== undefined
    ? (parseInputNum(patch.dual_instruction_hours, 'dual_instruction_hours') || 0)
