...
function preserveLocalTimeOnDate(originalStart, originalEnd, nextDate) {
  const start = new Date(originalStart);
  const end = new Date(originalEnd);
  const durationMs = Number.isFinite(end.getTime() - start.getTime()) && end > start
    ? end.getTime() - start.getTime()
    : 60 * 60 * 1000;
  const nextStart = wallClockToUtc(nextDate, timeHmFromDate(start));
  return {
    startTime: nextStart,
...
async function syncFlightRecord(client, bookingId, patch = {}) {
  ...
  if (patch.flight_date) {
    const { startTime, endTime } = preserveLocalTimeOnDate(
      bookingBefore.start_time,
      bookingBefore.end_time,
      flightDate
    );
...
async function syncFlightRecordFromInstructorHours(client, instructorHoursRow) {
  ...
  const bookingDate = bkRes.rows[0]?.start_time ? calendarDateFromDate(bkRes.rows[0].start_time) : null;
  if (entryDate && entryDate !== bookingDate) {
