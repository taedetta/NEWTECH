...
async function syncFlightRecord(client, bookingId, patch = {}) {
  ...
  const flightDate = normalizeFlightDate(patch.flight_date)
    || normalizeFlightDate(oldLog?.flight_date)
    || normalizeFlightDate(bookingBefore.start_time)
  ...
  const shiftedTimes = shiftedBookingTimesForFlightDate(
  ...
  if (shiftedTimes) {
    ...
    bookingVals.push(shiftedTimes.startIso);
    bookingSets.push(`end_time = $${bi++}`);
    bookingVals.push(shiftedTimes.endIso);
  }
