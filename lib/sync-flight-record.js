...
function normalizeDateOnly(value) {
  ...
  if (/^\d{4}-\d{2}-\d{2}$/.test(dateStr) && !raw.includes('T')) return dateStr;
  const parsed = new Date(raw);
  return Number.isNaN(parsed.getTime()) ? null : calendarDateFromDate(parsed);
}

function shiftedBookingTimesForFlightDate(booking, flightDate) {
  const targetDate = normalizeDateOnly(flightDate);
  if (!targetDate || !booking?.start_time || !booking?.end_time) return null;
  ...
  const currentDate = calendarDateFromDate(currentStart);
...
async function syncFlightRecord(client, bookingId, patch = {}) {
  ...
  const flightDate = normalizeDateOnly(patch.flight_date)
    || normalizeDateOnly(oldLog?.flight_date)
    || normalizeDateOnly(bookingBefore.start_time)
  ...
  const { aircraftChargeAmount, instructionChargeAmount } = resolveFlightCharges({
    ...
    dualHrs,
    hourlyRate: patch.allow_rate_overrides === true ? (patch.aircraft_rate_override ?? acRate?.hourly_rate) : acRate?.hourly_rate,
    instructorRate: patch.allow_rate_overrides === true ? (patch.instructor_rate_override ?? instrRate?.instructor_rate) : instrRate?.instructor_rate,
    aircraftChargeAmount: patch.allow_charge_overrides === true ? patch.aircraft_charge_amount : null,
    instructionChargeAmount: patch.allow_charge_overrides === true ? patch.instruction_charge_amount : null,
  });
  ...
  const shiftedTimes = patch.flight_date ? shiftedBookingTimesForFlightDate(bookingBefore, flightDate) : null;
  if (shiftedTimes) {
    bookingSets.push(`start_time = $${bi++}`);
    bookingVals.push(shiftedTimes.start_time);
    bookingSets.push(`end_time = $${bi++}`);
    bookingVals.push(shiftedTimes.end_time);
  }
...
async function syncFlightRecordFromInstructorHours(client, instructorHoursRow, options = {}) {
  ...
  const patch = {
    ...
    allow_rate_overrides: options.allowRateOverrides === true,
