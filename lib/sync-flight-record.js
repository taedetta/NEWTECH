...
function resolveFlightDate({ patchFlightDate, oldLogFlightDate, bookingStartTime, fallbackDate = new Date() }) {
  return normalizeDateOnly(patchFlightDate)
    || normalizeDateOnly(oldLogFlightDate)
    || (bookingStartTime ? calendarDateFromDate(bookingStartTime) : null)
    || calendarDateFromDate(fallbackDate);
}

function moveBookingWindowToDate(booking, flightDate) {
  const start = new Date(booking.start_time);
  const end = new Date(booking.end_time);
  if (Number.isNaN(start.getTime()) || Number.isNaN(end.getTime()) || end <= start) {
    throw new Error('Cannot move booking with invalid start/end times');
  }
...
async function syncFlightRecord(client, bookingId, patch = {}) {
  ...
  const flightDate = resolveFlightDate({
    patchFlightDate: patch.flight_date,
    oldLogFlightDate: oldLog?.flight_date,
    bookingStartTime: bookingBefore.start_time,
  });
  ...
  if (patch.flight_date) {
    const { startTime, endTime } = moveBookingWindowToDate(bookingBefore, flightDate);
