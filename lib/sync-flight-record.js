...
function shiftedBookingTimesForFlightDate(bookingBefore, flightDate) {
  const nextDate = normalizeCalendarDate(flightDate);
  if (!nextDate || !bookingBefore?.start_time || !bookingBefore?.end_time) return null;
  ...
  if (!currentDate || currentDate === nextDate) return null;

  const durationMs = currentEnd.getTime() - currentStart.getTime();
  if (durationMs <= 0) return null;

  const shiftedStart = wallClockToUtc(nextDate, timeHmFromDate(currentStart));
...
async function syncFlightRecord(client, bookingId, patch = {}) {
  ...
  const flightDate = normalizeCalendarDate(patch.flight_date)
    || normalizeCalendarDate(oldLog?.flight_date)
  ...
  const shiftedBookingTimes = shiftedBookingTimesForFlightDate(bookingBefore, patch.flight_date);
  if (shiftedBookingTimes) {
    bookingSets.push(`start_time = $${bi++}`);
    bookingVals.push(shiftedBookingTimes.startTime);
    bookingSets.push(`end_time = $${bi++}`);
    bookingVals.push(shiftedBookingTimes.endTime);
  }
