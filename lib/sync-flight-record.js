...
function moveTimestampToDate(value, targetDate) {
  const d = new Date(value);
  if (!targetDate || Number.isNaN(d.getTime())) return null;
  return new Date(`${targetDate}${d.toISOString().slice(10)}`);
}
...
async function syncFlightRecord(client, bookingId, patch = {}) {
  ...
  const currentBookingDate = dateOnly(bookingBefore.start_time);
  if (patch.flight_date && flightDate !== currentBookingDate) {
    const currentStart = new Date(bookingBefore.start_time);
    const currentEnd = new Date(bookingBefore.end_time);
    const startTime = moveTimestampToDate(bookingBefore.start_time, flightDate)
      || new Date(flightDate + 'T12:00:00Z');
    const durationMs = !Number.isNaN(currentStart.getTime()) && !Number.isNaN(currentEnd.getTime())
