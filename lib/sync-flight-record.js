...
const { applyAircraftMeterReadings, getMeterHobbs, getMeterTach } = require('./aircraft-meter');
...
async function syncFlightRecord(client, bookingId, patch = {}) {
  ...
  if (patch.flight_date) {
    const startTime = new Date(flightDate + 'T12:00:00Z');
  ...
  if (booking.status === 'completed' && !booking.billing_voided && booking.aircraft_id && hEnd != null) {
    const oldHobbsEnd = parseNum(oldLog?.hobbs_end ?? bookingBefore.hobbs_end);
    const oldTachEnd = parseNum(oldLog?.tach_end ?? bookingBefore.tach_end);
    ...
    const currentHobbs = getMeterHobbs(aircraft);
    const currentTach = getMeterTach(aircraft);
    const editsCurrentHobbs = oldHobbsEnd != null && currentHobbs != null && Math.abs(currentHobbs - oldHobbsEnd) <= 0.1;
    const editsCurrentTach = oldTachEnd != null && currentTach != null && Math.abs(currentTach - oldTachEnd) <= 0.1;
    const advancesHobbs = currentHobbs == null || hEnd > currentHobbs;
    const advancesTach = tEnd != null && (currentTach == null || tEnd > currentTach);
    if (editsCurrentHobbs || editsCurrentTach || advancesHobbs || advancesTach) {
      await applyAircraftMeterReadings(client, booking.aircraft_id, {
        hobbsEnd: hEnd,
        tachEnd: tEnd != null ? tEnd : undefined,
        bookingId,
        source: 'flight_edit',
      });
    }
