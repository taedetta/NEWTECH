...
function movedBookingTimes(booking, flightDate) {
  const start = new Date(booking.start_time);
  const end = new Date(booking.end_time);
  if (Number.isNaN(start.getTime()) || Number.isNaN(end.getTime()) || end <= start) {
    return null;
  }
  const movedStart = wallClockToUtc(flightDate, timeHmFromDate(start));
  const movedEnd = new Date(movedStart.getTime() + (end.getTime() - start.getTime()));
  return { startTime: movedStart, endTime: movedEnd };
}
...
async function syncFlightRecord(client, bookingId, patch = {}) {
  ...
  const shouldRecalculateCharges = patch.hobbs_start !== undefined
    || patch.hobbs_end !== undefined
    || patch.dual_instruction_hours !== undefined
    || patch.lesson_type !== undefined
    || patch.aircraft_rate_override !== undefined
    || patch.instructor_rate_override !== undefined
    || patch.aircraft_charge_amount !== undefined
    || patch.instruction_charge_amount !== undefined;
  ...
  const { aircraftChargeAmount, instructionChargeAmount } = resolveFlightCharges({
    ...
    aircraftChargeAmount: shouldRecalculateCharges ? patch.aircraft_charge_amount : oldLog?.aircraft_charge_amount,
    instructionChargeAmount: shouldRecalculateCharges ? patch.instruction_charge_amount : oldLog?.instruction_charge_amount,
  });
  ...
  if (requestedFlightDate) {
    const moved = movedBookingTimes(bookingBefore, flightDate);
    if (moved) {
      bookingSets.push(`start_time = $${bi++}`);
      bookingVals.push(moved.startTime.toISOString());
      bookingSets.push(`end_time = $${bi++}`);
      bookingVals.push(moved.endTime.toISOString());
    }
  }
...
async function syncFlightRecordFromInstructorHours(client, instructorHoursRow) {
  ...
  const entryDate = dateOnly(instructorHoursRow.entry_date);
  if (entryDate && entryDate !== dateOnly(bkRes.rows[0]?.start_time)) {
