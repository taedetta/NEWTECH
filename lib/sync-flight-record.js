...
function dateOnlyString(value) {
  ...
  return calendarDateFromDate(value);
}

function bookingTimesForFlightDate(booking, flightDate) {
  if (!booking?.start_time || !booking?.end_time || !flightDate) return null;
  const oldStart = new Date(booking.start_time);
  const oldEnd = new Date(booking.end_time);
  if (Number.isNaN(oldStart.getTime()) || Number.isNaN(oldEnd.getTime())) return null;
  const durationMs = oldEnd.getTime() - oldStart.getTime();
  if (durationMs <= 0) return null;
  const movedStart = wallClockToUtc(dateOnlyString(flightDate), timeHmFromDate(oldStart));
  return {
    startTime: movedStart.toISOString(),
    endTime: new Date(movedStart.getTime() + durationMs).toISOString(),
  };
}

function buildPatchFromInstructorHoursRow(instructorHoursRow, bookingRow = {}) {
  const entryDate = dateOnlyString(instructorHoursRow.entry_date);
  const currentFlightDate = dateOnlyString(bookingRow.flight_date)
    || (bookingRow.start_time ? calendarDateFromDate(bookingRow.start_time) : null);
  const patch = {
    dual_instruction_hours: instructorHoursRow.instruction_hours,
    preserve_instructor_hours_rates: true,
  };
  if (entryDate && entryDate !== currentFlightDate) {
    patch.flight_date = entryDate;
  }
  if (instructorHoursRow.allow_rate_overrides) {
    patch.instructor_rate_override = instructorHoursRow.instructor_rate;
    patch.aircraft_rate_override = instructorHoursRow.aircraft_rate;
  }
  return patch;
}
...
async function syncFlightRecord(client, bookingId, patch = {}) {
  ...
  const requestedFlightDate = dateOnlyString(patch.flight_date);
  const currentBookingDate = bookingBefore.start_time ? calendarDateFromDate(bookingBefore.start_time) : null;
  const flightDate = requestedFlightDate
    || dateOnlyString(oldLog?.flight_date)
    || currentBookingDate
  ...
  if (requestedFlightDate && requestedFlightDate !== currentBookingDate) {
    const moved = bookingTimesForFlightDate(bookingBefore, requestedFlightDate);
    if (!moved) throw new Error('Cannot move booking to requested flight date');
    bookingSets.push(`start_time = $${bi++}`);
    bookingVals.push(moved.startTime);
    bookingSets.push(`end_time = $${bi++}`);
    bookingVals.push(moved.endTime);
  }
...
async function syncFlightRecordFromInstructorHours(client, instructorHoursRow) {
  ...
  const bkRes = await client.query(
    `SELECT b.hobbs_start, b.hobbs_end, b.start_time, b.end_time, fl.flight_date
     FROM bookings b
  ...
  const patch = buildPatchFromInstructorHoursRow(instructorHoursRow, bookingRow);

  const acHrs = parseFloat(instructorHoursRow.aircraft_hours) || 0;
  const hStart = parseNum(bookingRow.hobbs_start);
