...
const { applyAircraftMeterReadings, reconcileAircraftMeterForFlightEdit } = require('./aircraft-meter');
const { parseStrictNumber } = require('./strict-number');
...
function parsePatchNum(value, fieldName) {
  if (value == null || value === '') return null;
  const parsed = parseStrictNumber(value, fieldName, { required: false, allowEmpty: true });
...
function shiftBookingTimesToFlightDate(booking, flightDate) {
  ...
  const currentDate = dateOnly(booking?.start_time);
  ...
  const shiftedStart = new Date(`${targetDate}T${currentStart.toISOString().slice(11)}`);
...
async function syncFlightRecord(client, bookingId, patch = {}) {
  const bkRes = await client.query('SELECT * FROM bookings WHERE id = $1 AND source = $2 FOR UPDATE', [bookingId, getAppEnv()]);
  if (!bkRes.rows.length) throw new Error('Booking not found');
  const bookingBefore = bkRes.rows[0];

  const logRes = await client.query('SELECT * FROM flight_logs WHERE booking_id = $1 AND source = $2 FOR UPDATE', [bookingId, getAppEnv()]);
  ...
  const hStart = patch.hobbs_start !== undefined
    ? parsePatchNum(patch.hobbs_start, 'hobbs_start')
    : parseNum(oldLog?.hobbs_start ?? bookingBefore.hobbs_start);
  const hEnd = patch.hobbs_end !== undefined
    ? parsePatchNum(patch.hobbs_end, 'hobbs_end')
    : parseNum(oldLog?.hobbs_end ?? bookingBefore.hobbs_end);
  const tStart = patch.tach_start !== undefined
    ? parsePatchNum(patch.tach_start, 'tach_start')
    : parseNum(oldLog?.tach_start ?? bookingBefore.tach_start);
  const tEnd = patch.tach_end !== undefined
    ? parsePatchNum(patch.tach_end, 'tach_end')
    : parseNum(oldLog?.tach_end ?? bookingBefore.tach_end);
  const hobbsTouched = patch.hobbs_start !== undefined || patch.hobbs_end !== undefined;
  if (hobbsTouched && (hStart == null || hEnd == null)) {
    throw badRequest('hobbs_start and hobbs_end are required together');
  }
  if (hStart != null && hEnd != null && hEnd <= hStart) {
    throw badRequest('hobbs_end must be greater than hobbs_start');
  }
  const tachTouched = patch.tach_start !== undefined || patch.tach_end !== undefined;
  if (tachTouched && (tStart == null) !== (tEnd == null)) {
    throw badRequest('tach_start and tach_end are required together');
  }
  if (tStart != null && tEnd != null && tEnd <= tStart) {
    throw badRequest('tach_end must be greater than tach_start');
  }

  const hobbsDelta = !hobbsTouched && oldLog?.hobbs_delta != null
    ? parseFloat(oldLog.hobbs_delta)
    : hobbsDeltaFrom(hStart, hEnd);
  const tachDelta = !tachTouched && oldLog?.tach_delta != null
    ? parseFloat(oldLog.tach_delta)
    : tachDeltaFrom(tStart, tEnd);

  const dualHrs = patch.dual_instruction_hours !== undefined
    ? (parsePatchNum(patch.dual_instruction_hours, 'dual_instruction_hours') || 0)
  ...
  const requestedFlightDate = dateOnly(patch.flight_date);
  const flightDate = requestedFlightDate
    || dateOnly(oldLog?.flight_date)
    || dateOnly(bookingBefore.start_time)
  ...
  const aircraftChargeOverride = patch.aircraft_charge_amount !== undefined
    ? parsePatchNum(patch.aircraft_charge_amount, 'aircraft_charge_amount')
    : undefined;
  const instructionChargeOverride = patch.instruction_charge_amount !== undefined
    ? parsePatchNum(patch.instruction_charge_amount, 'instruction_charge_amount')
    : undefined;
  const { aircraftChargeAmount, instructionChargeAmount } = resolveFlightCharges({
    ...
    aircraftChargeAmount: aircraftChargeOverride,
    instructionChargeAmount: instructionChargeOverride,
  });
  ...
  const shiftedTimes = requestedFlightDate
    ? shiftBookingTimesToFlightDate(bookingBefore, requestedFlightDate)
    : null;
  if (shiftedTimes) {
    bookingSets.push(`start_time = $${bi++}`);
    bookingVals.push(shiftedTimes.startTime.toISOString());
    bookingSets.push(`end_time = $${bi++}`);
    bookingVals.push(shiftedTimes.endTime.toISOString());
  }
  if (bookingSets.length) {
    bookingSets.push('updated_at = NOW()');
    bookingVals.push(bookingId);
    await client.query(
      `UPDATE bookings SET ${bookingSets.join(', ')} WHERE id = $${bi} AND source = $${bi + 1}`,
      [...bookingVals, getAppEnv()]
    );
  }

  const bkAfter = await client.query('SELECT * FROM bookings WHERE id = $1 AND source = $2', [bookingId, getAppEnv()]);
  ...
  if (oldLog) {
    await client.query(
      `UPDATE flight_logs SET
       ...
       WHERE booking_id = $16 AND source = $15`,
      [
        ...
        booking.booking_type || 'dual', getAppEnv(), bookingId,
      ]
    );
    ...
    await client.query(
      `INSERT INTO flight_logs
         (booking_id, aircraft_id, student_id, instructor_id, booking_type,
          ...
          dual_instruction_hours, submitted_by, aircraft_charge_amount, instruction_charge_amount, source)
  ...
  if (booking.status === 'completed' && hobbsDelta != null) {
    const oldStudentId = oldLog?.student_id != null ? oldLog.student_id : (patch.previous_student_id ?? bookingBefore.student_id);
    const oldInstructorId = oldLog?.instructor_id != null ? oldLog.instructor_id : (patch.previous_instructor_id ?? bookingBefore.instructor_id);
    ...
    if (booking.aircraft_id && hEnd != null) {
      await reconcileAircraftMeterForFlightEdit(client, booking.aircraft_id, bookingId, {
        oldHobbsEnd: oldLog?.hobbs_end ?? bookingBefore.hobbs_end,
        oldTachEnd: oldLog?.tach_end ?? bookingBefore.tach_end,
        newHobbsEnd: hEnd,
        newTachEnd: tEnd,
        source: 'flight_record_sync_correction',
      });
      await applyAircraftMeterReadings(client, booking.aircraft_id, {
        hobbsEnd: hEnd,
        tachEnd: tEnd,
        bookingId,
        source: 'flight_record_sync',
      });
    }
  }
  ...
  if (booking.instructor_id && booking.status === 'completed') {
    ...
    const ihExisting = await client.query('SELECT * FROM instructor_hours WHERE booking_id = $1 AND source = $2', [bookingId, getAppEnv()]);
  ...
  const flAfter = await client.query('SELECT * FROM flight_logs WHERE booking_id = $1 AND source = $2', [bookingId, getAppEnv()]);
...
async function syncFlightRecordFromInstructorHours(client, instructorHoursRow) {
  ...
  const bkRes = await client.query(
    'SELECT hobbs_start, hobbs_end FROM bookings WHERE id = $1 AND source = $2',
    [instructorHoursRow.booking_id, getAppEnv()]
  );
  const patch = {
