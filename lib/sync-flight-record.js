...
async function syncFlightRecord(client, bookingId, patch = {}) {
  ...
  const currentBookingDate = bookingBefore.start_time
    ? calendarDateFromDate(bookingBefore.start_time)
    : null;
  const requestedFlightDate = patch.flight_date !== undefined
    ? calendarDateOnly(patch.flight_date)
    : null;
  const flightDate = requestedFlightDate
    || calendarDateOnly(oldLog?.flight_date)
    || currentBookingDate
  ...
  if (requestedFlightDate && requestedFlightDate !== currentBookingDate && bookingBefore.start_time) {
    const currentStart = new Date(bookingBefore.start_time);
    const currentEnd = bookingBefore.end_time ? new Date(bookingBefore.end_time) : null;
    const durationMs = currentEnd && !Number.isNaN(currentEnd.getTime()) && currentEnd > currentStart
      ? currentEnd.getTime() - currentStart.getTime()
      : 60 * 60 * 1000;
    const startTime = wallClockToUtc(requestedFlightDate, timeHmFromDate(currentStart));
