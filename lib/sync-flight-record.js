...
const { dateOnly, shiftBookingTimesToFlightDate } = require('./flight-date-shift');
...
async function syncFlightRecord(client, bookingId, patch = {}) {
  ...
  const shiftedTimes = requestedFlightDate
    ...
    : null;
  if (shiftedTimes) {
    const { startTime, endTime } = shiftedTimes;
...
async function syncFlightRecordFromInstructorHours(client, instructorHoursRow, options = {}) {
  ...
  if (options.entryDateChanged) {
    patch.flight_date = instructorHoursRow.entry_date;
  }
