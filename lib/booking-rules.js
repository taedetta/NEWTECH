...
async function runPreflightChecks(client, { aircraft_id, student_id, instructor_id, start_time, end_time, booking_type, lesson_type, local_date, local_start, local_end, skipPastTimeCheck, skipInstructorAvailability }, userRole) {
  const policy = await getPolicySettings();
  const start = new Date(start_time);
  const end = new Date(end_time);
  const isAdmin = ['owner', 'admin'].includes(userRole);
  const { errors, warnings } = validateBookingTimes({
    start, end, local_start, local_end, policy, userRole, isAdmin, booking_type, lesson_type, skipPastTimeCheck,
  });
