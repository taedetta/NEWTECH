...
const { todayCalendarDate: todayEasternCalendarDate } = require('./school-timezone');
...
function todayCalendarDate() {
  return todayEasternCalendarDate();
}
...
function canBookPastTimes(userRole, isAdmin) {
  return isAdmin || ['owner', 'admin'].includes(userRole);
}

function validateBookingTimes({ start, end, local_start, local_end, policy, userRole, isAdmin, booking_type, lesson_type, skipDiscoveryDurationCheck, skipPastTimeCheck }) {
  const errors = [];
  const warnings = [];
  ...
  if (isDiscoveryLessonType(lesson_type) && !skipDiscoveryDurationCheck) {
    if (Math.abs(durationMin - 20) > 0.5) {
      errors.push('Discovery flights must be booked in exactly 20-minute blocks');
    }
    if (local_start) {
      const startMin = parseInt(String(local_start).split(':')[1], 10) || 0;
      if (startMin % 20 !== 0) {
        errors.push('Discovery flight start times must use 20-minute intervals (e.g. 9:00, 9:20, 9:40)');
      }
    }
  ...
  if (!skipPastTimeCheck && !canBookPastTimes(userRole, isAdmin) && start.getTime() < now.getTime() - graceMs) {
...
async function checkGroundingSquawk(client, aircraftId) {
  ...
  const result = await db.query(
    `SELECT id, description FROM squawks
     WHERE aircraft_id = $1 AND source = $2 AND status IN ('open','reviewed') AND severity = 'grounding'
     LIMIT 1`,
    [aircraftId, getAppEnv()]
  );
...
async function runPreflightChecks(client, { aircraft_id, student_id, instructor_id, start_time, end_time, booking_type, lesson_type, local_date, local_start, local_end, skipPastTimeCheck, skipInstructorAvailability, skipDiscoveryDurationCheck }, userRole) {
  ...
  const { errors, warnings } = validateBookingTimes({
    ...
    skipPastTimeCheck, skipDiscoveryDurationCheck,
  });

  if (aircraft_id) {
    const ac = await (client || pool).query(
      'SELECT tail_number, status, current_tach, total_tach_hours, next_100hr_due, next_annual_due FROM aircraft WHERE id = $1 AND source = $2',
      [aircraft_id, getAppEnv()]
    );
    if (ac.rows.length === 0) errors.push('Aircraft not found');
    else {
      const a = ac.rows[0];
      if (a.status !== 'available' && !isAdmin) errors.push(`Aircraft is ${a.status}`);
  ...
  if (instructor_id && !isAdmin && !skipInstructorAvailability) {
    const calendarDate = local_date || null;
    const localOpts = (local_start && local_end)
      ? {
          localDate: calendarDate || start.toISOString().slice(0, 10),
