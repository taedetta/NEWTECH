...
/** Normalize DATE / string to YYYY-MM-DD (node-pg may return Date objects). */
...
function isAllDayDowntime(row) {
  if (!row) return false;
  if (timeToHHMM(row.start_time) && timeToHHMM(row.end_time)) return false;
...
function downtimeBounds(row) {
  ...
  const startDate = normalizeDateInput(row.start_date);
  const endDate = normalizeDateInput(row.end_date);
...
function downtimeTouchesDate(row, dateStr) {
  ...
  const start = normalizeDateInput(row.start_date);
  const end = normalizeDateInput(row.end_date);
...
async function findBookingsOverlappingDowntime(db, aircraftId, downtimeRow) {
  ...
  if (!bounds || Number.isNaN(bounds.start.getTime()) || Number.isNaN(bounds.end.getTime())) return [];
  ...
  const result = await pool.query(
    ...
    [aircraftId, bounds.start.toISOString(), bounds.end.toISOString(), getAppEnv()]
  );
