...
const REQUIRED_EMAIL_TYPE_KEYS = [
  'password_reset',
  'account_approved',
  'account_rejected',
  'signup_pending',
...
const REQUIRED_EMAIL_TYPES = new Set(REQUIRED_EMAIL_TYPE_KEYS);

function isRequiredEmailType(type) {
  return REQUIRED_EMAIL_TYPES.has(type);
}

function isConfigurableEmailType(type) {
  return !!EMAIL_TYPES[type] && !isRequiredEmailType(type);
}
