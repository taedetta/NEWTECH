...
const { formatDate, formatTime, calendarDateFromDate, wallClockToUtc, addCalendarDays } = require('./school-timezone');
...
function instructorBriefingEmail({ instructorName, dateLabel, flights, atRiskStudents, squawks, weatherSummary }) {
  ...
  const flightLines = flights.length
    ? flights.map((f) => {
        const t = `${formatTime(f.start_time, { hour: '2-digit', minute: '2-digit' })}–${formatTime(f.end_time, { hour: '2-digit', minute: '2-digit' })} ET`;
...
async function buildInstructorBriefing(instructorId) {
  const dateStr = calendarDateFromDate(new Date());
  const startOfDay = wallClockToUtc(dateStr, '00:00');
  const endOfDay = new Date(wallClockToUtc(addCalendarDays(dateStr, 1), '00:00').getTime() - 1);
  ...
  for (const instr of instructors.rows) {
    try {
      ...
      await sendEmailToUser(
        instr.id, instr.email, EMAIL_TYPES.instructor_briefing,
...
function startInstructorBriefingScheduler(pool) {
  ...
  function scheduleNext() {
    const now = new Date();
    const todayStr = calendarDateFromDate(now);
    let next = wallClockToUtc(todayStr, `${String(HOUR_ET).padStart(2, '0')}:00`);
    if (now >= next) {
      next = wallClockToUtc(addCalendarDays(todayStr, 1), `${String(HOUR_ET).padStart(2, '0')}:00`);
    }
