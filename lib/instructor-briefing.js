...
function instructorBriefingEmail({ instructorName, dateLabel, flights, atRiskStudents, squawks, weatherSummary }) {
  ...
  const html = `
    <div style="font-family:Arial,sans-serif;max-width:560px;color:#1e293b">
      ...
      <p style="color:#64748b;margin:0 0 1rem">${dateLabel} — your daily flight school briefing (all times Central)</p>
  ...
  const text = `Daily briefing for ${instructorName} — ${dateLabel}. ${flights.length} flight(s) scheduled today (Central Time). Open ${getAppUrl()}/app`;
