...
async function findMatchingFlights({ instructorId, entryDate, aircraftId, studentName, bookingId }) {
  if (bookingId) {
    const byBooking = await pool.query(`
      ...
      WHERE fl.booking_id = $1 AND fl.source = $2 AND b.source = $2
    `, [bookingId, getAppEnv()]);
  ...
  `;
  const params = [entryDate, instructorId, getAppEnv()];
  if (aircraftId) {
...
async function auditInstructorHoursEntry({ instructorId, entryDate, aircraftId, aircraftHours, instructionHours, studentName, bookingId }) {
  ...
  if (flights.length === 0) {
    return {
      ok: true,
      status: bookingId ? 'ok' : 'unmatched',
      message: bookingId ? null : 'No matching completed flight log for this date/aircraft — verify entry manually',
      details: { flights: 0 },
    };
  }
...
async function listHoursAuditFlags({ instructorId } = {}) {
  ...
  const ihResult = await pool.query(`
    ...
    WHERE ih.source = $1 AND ih.audit_status IN ('flagged', 'unmatched') ${extra}
