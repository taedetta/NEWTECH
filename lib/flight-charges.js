...
function billableInstructionHours({ bookingType, hobbsDelta, dualHrs }) {
  const hobbs = parseFloat(hobbsDelta) || 0;
  const dual = parseFloat(dualHrs) || 0;
  return dual > 0 ? dual : (bookingType === 'dual' ? hobbs : 0);
}
...
function computeFlightCharges({ lessonType, bookingType, hobbsDelta, dualHrs, hourlyRate, instructorRate }) {
  ...
  const hobbs = parseFloat(hobbsDelta) || 0;
  const dual = billableInstructionHours({ bookingType, hobbsDelta: hobbs, dualHrs });
...
module.exports = { DISCOVERY_FLAT_CHARGE, billableInstructionHours, computeFlightCharges, resolveFlightCharges };
