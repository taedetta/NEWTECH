...
function shouldBillInstruction(lessonType, bookingType) {
  if (bookingType) return bookingType === 'dual';
  return /\bdual\b/i.test(String(lessonType || ''));
}

function computeFlightCharges({ lessonType, bookingType, hobbsDelta, dualHrs, hourlyRate, instructorRate }) {
  ...
  const instructionHours = dual > 0 ? dual : (shouldBillInstruction(lessonType, bookingType) ? hobbs : 0);
  const instructionChargeAmount = (instructionHours > 0 && iRate)
    ? Math.round(instructionHours * iRate * 100) / 100
    : 0;
...
function resolveFlightCharges({
  ...
  const computed = computeFlightCharges({ lessonType, bookingType, hobbsDelta, dualHrs, hourlyRate, instructorRate });
  if (isDiscoveryLessonType(lessonType)) {
    return computed;
  }
