...
const { formatDateLongCT, formatTimeCT } = require('./timezone-ct');
...
async function runPreflightReminders(pool) {
  ...
  for (const booking of result.rows) {
    try {
      const flightDate = formatDateLongCT(booking.start_time);
      const flightTime = formatTimeCT(booking.start_time);
      ...
      try {
        ...
        const pushPayload = {
          ...
          body: `${booking.tail_number} at ${formatTimeCT(new Date(booking.start_time))}`,
          link: '/app/schedule',
