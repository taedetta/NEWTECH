...
const { preflightReminderEmailStudent, preflightReminderEmailInstructor } = require('../email-templates');
const { sendEmailToUser, EMAIL_TYPES } = require('./notification-prefs');
const { formatDate, formatTime } = require('./school-timezone');
...
function formatTimeET(date) {
  return formatTime(date, { hour: '2-digit', minute: '2-digit', hour12: true });
}
...
async function runPreflightReminders(pool) {
  ...
  for (const booking of result.rows) {
    try {
      const flightDate = formatDateET(new Date(booking.start_time));
      const flightTime = formatTimeET(new Date(booking.start_time));
      ...
      if (booking.student_email && booking.student_name) {
        ...
        await sendEmailToUser(
          booking.student_id, booking.student_email, EMAIL_TYPES.preflight_reminder,
          subject, html, text
        );
      ...
      if (booking.instructor_id && booking.instructor_email && booking.instructor_name) {
        ...
        await sendEmailToUser(
          booking.instructor_id, booking.instructor_email, EMAIL_TYPES.preflight_reminder,
          subject, html, text
        );
      ...
      try {
        ...
        const pushPayload = {
          ...
          body: `${booking.tail_number} at ${formatTimeET(new Date(booking.start_time))} ET`,
          link: '/app/schedule',
          notification_type: 'preflight_reminder',
        };
        if (booking.student_id) await sendPushToUser(booking.student_id, pushPayload);
        if (booking.instructor_id) await sendPushToUser(booking.instructor_id, pushPayload);
      } catch (_) { /* optional */ }
...
function startPreflightReminderScheduler(pool) {
  ...
  if (isStaging()) {
    console.log('[reminder-email] Staging — pre-flight reminder scheduler disabled');
    return;
  }
