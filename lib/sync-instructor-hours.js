 ...
 * Discovery flights store effective rates so total billed = $185 flat.
 */
async function syncInstructorHoursFromFlight(client, { booking, hobbsFlown, dualHrs, flightDate, studentName }) {
  ...
  if (isDiscoveryLessonType(booking.lesson_type)) {
    ...
    acRate = acRateRow ? parseFloat(acRateRow.hourly_rate) : null;
    instrRate = instrRateRow ? parseFloat(instrRateRow.instructor_rate) : null;
  }
