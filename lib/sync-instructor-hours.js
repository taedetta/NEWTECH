...
async function syncInstructorHoursFromFlight(client, {
  ...
  const existing = await client.query('SELECT id FROM instructor_hours WHERE booking_id = $1 AND source = $2', [booking.id, getAppEnv()]);
  const fields = [
    booking.instructor_id,
  ...
  if (existing.rows.length > 0) {
    await client.query(
      `UPDATE instructor_hours SET
         ...
         audit_status = $11, audit_message = $12, source = $13, updated_at = NOW()
       WHERE booking_id = $10 AND source = $13`,
      [...fields, getAppEnv()]
    );
  ...
  const ins = await client.query(
    `INSERT INTO instructor_hours
       (instructor_id, aircraft_id, entry_date, aircraft_hours, instruction_hours,
        aircraft_rate, instructor_rate, notes, student_name, booking_id, audit_status, audit_message, source)
