...
function verifyUnsubscribeToken(token) {
  try {
    ...
    if (payload.aud === 'email-unsub') {
      // Current transitional tokens may have the audience but not a signed type.
      return { userId: payload.uid, type: safeType, legacyTypeInQuery: !safeType };
    }
    if (!safeType) return null;
    return { userId: payload.uid, type: safeType, legacyTypeInQuery: false };
  } catch (_) {
    return null;
  }
}
