...
const { EMAIL_TYPES, TYPE_LABELS, isEmailTypeMutable } = require('./email-types');

const JWT_SECRET = process.env.JWT_SECRET || 'REDACTED';

function normalizeUnsubscribeType(type) {
  return type === 'all' || isEmailTypeMutable(type) ? type : 'all';
}
...
function verifyUnsubscribeToken(token) {
  try {
    ...
    if (!payload.type || type !== payload.type) return null;
