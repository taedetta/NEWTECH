...
function isUnsubscribableEmailType(type) {
  return type === 'all' || (!!EMAIL_TYPES[type] && !isRequiredEmailType(type));
}

/** Token binds both the user and preference scope so query-string tampering cannot disable other types. */
function signUnsubscribeToken(userId, type = 'all') {
  const safeType = isUnsubscribableEmailType(type) ? type : 'all';
...
function verifyUnsubscribeToken(token) {
  try {
    ...
    if (!payload?.uid || payload.aud !== 'email-unsub' || !isUnsubscribableEmailType(payload.typ)) return null;
    return { userId: payload.uid, emailType: payload.typ };
  } catch (_) {
    return null;
  }
}
