...
function normalizeUnsubscribeType(type) {
  ...
  if (EMAIL_TYPES[type] && !REQUIRED_EMAIL_TYPES.has(type)) return type;
  return 'all';
}
...
function verifyUnsubscribeToken(token) {
  try {
    ...
    const emailType = payload.typ;
    if (emailType !== 'all' && (!EMAIL_TYPES[emailType] || REQUIRED_EMAIL_TYPES.has(emailType))) return null;
    return { userId: payload.uid, emailType };
  } catch (_) {
    return null;
  }
}
