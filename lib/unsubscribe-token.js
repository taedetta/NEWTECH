...
/** Token binds the user and exact unsubscribe scope; query params are display-only. */
function signUnsubscribeToken(userId, type = 'all') {
  const normalizedType = normalizeUnsubscribeType(type) || 'all';
  return jwt.sign({ uid: userId, typ: normalizedType, aud: 'email-unsub' }, JWT_SECRET, { expiresIn: '365d' });
}
